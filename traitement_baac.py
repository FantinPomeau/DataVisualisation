...
accident = pd.read_csv('C:/Users/hugo.razafindralambo/OneDrive - ADDACTIS GROUP/Bureau/Personnel/Datavis/baac/accident.csv')
...
cntr_dep = gpd.read_file("C:/Users/hugo.razafindralambo/Downloads/Geojson/departements.geojson")
cntr_dep.rename(columns = {"code" : "dep", "nom":"Nom_dep","geometry":"geometry_dep"}, inplace = True)

cntr_com_2021 = gpd.read_file("C:/Users/hugo.razafindralambo/Downloads/commune-frmetdrom-2021/COMMUNE_FRMETDROM.shp")
...
def graph_datetime(time):
    ...
    fig = px.line(
        ...
        title="Evolution temporelle du nombre d'accident en 2021.",
...
def plot_accidents_by_surface(loc):
    ...
    fig = px.bar(type_route_counts, x='count', y='surface', orientation='h',
                 title=f'Nombre d\'accidents par état de la surface de la route dans {name}: {loc}',
                 color_discrete_sequence=['turquoise'] * len(type_route_counts))
...
def plot_accidents_by_vitesse(loc):
    ...
    fig = px.bar(type_route_counts, x='count', y='vitesse_autorisee', orientation='h',
                 title=f'Nombre d\'accidents par vitesse autorisée (km/h) dans {name}: {loc}',
                 color_discrete_sequence=['turquoise'] * len(type_route_counts))
